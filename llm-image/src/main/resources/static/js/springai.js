...
springai.addUserQuestion = function (question, chatPanelId) {
  ...
  `;
  document.querySelector(`#${chatPanelId}`).innerHTML += html;
  //document.querySelector(`#${chatPanelId}`).insertAdjacentHTML('beforeend', html);
...
springai.printAnswerJson = async function (jsonString, uuid, chatPanelId) {
  ...
  document.querySelector(`#${uuid}`).innerHTML = "<pre>" + prettyJson + "</pre>";
  springai.scrollToHeight(chatPanelId);
};
...
springai.scrollToHeight = function (chatPanelId) {
  ...
  setTimeout(() => {
    const chatPanelElement = document.querySelector(`#${chatPanelId}`);
...
springai.setSpinner = function (spinnerId, status) {
  if (status) {
    document.querySelector(`#${spinnerId}`).classList.remove("d-none");
  } else {
    document.querySelector(`#${spinnerId}`).classList.add("d-none");
  }
};
...
vision.previewImage = function (fileId, chatPanelId) {
  ...
  if (file) {
    ...
    `;
    document.querySelector(`#${chatPanelId}`).innerHTML += previewHtml;

    const reader = new FileReader();
    reader.onload = function (e) {
      const preview = document.querySelector(`#${uuid}`);
...
vision.downloadBase64Image = function (base64Src, filename) {
